...
### 🛠 修復
- 匯出/匯入/清空：新增 `storeManager.exportData / importData / clearAll`，整合 `electronAPI` 對話框與檔案讀寫，成功匯出 JSON、支援匯入覆蓋。
- 修正「匯出資料失敗：exportData is not a function」問題。

### ✨ 體驗優化
- 狀態徽章：僅在「異常」(disconnected/error) 才顯示；`connecting/connected` 一律隱藏，避免干擾。
- 訊息框：標題改為「訊息」、內文置中、整體尺寸更緊湊。
- 隱藏「股票 API 控制面板」測試區塊（不影響功能）。

### 📈 股票名稱查詢穩定性
- 即時查詢：輸入 2 字以上，300ms 節流；加入防重入鎖與 3 秒超時回退，避免卡在「查詢中…」。
- 全域委派保底：就算初始化時序有落差，也能觸發查詢。
